...
public class RandomValidateCode {
    ...
    private String randString = "234567ACDEFGHJKMNPQRSTUVWXYZ";//随机产生的字符串
    ...
    public String getValidateCode(HttpSession session) {
    	return (String)session.getAttribute(SessionUtil.RANDOMCODEKEY);
    }
    
    public boolean checkValidateCode(String yourInput, HttpSession session) {
    	String valid = (String)session.getAttribute(SessionUtil.RANDOMCODEKEY);
    	if (yourInput != null && !yourInput.equals("") && yourInput.toUpperCase().equals(valid)) {
    		return true;
    	}
    	return false;
    }
